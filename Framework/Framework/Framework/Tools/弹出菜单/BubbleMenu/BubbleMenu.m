...
typedef enum {
    ...
    BubbleMenuViewArrowDirectionNone,
    BubbleMenuViewArrowDirectionUp,
    BubbleMenuViewArrowDirectionDown,
    BubbleMenuViewArrowDirectionLeft,
    BubbleMenuViewArrowDirectionRight,

} BubbleMenuViewArrowDirection;


@interface BubbleMenuView : UIView
...
@interface BubbleMenuOverlay : UIView
...
#pragma mark - BubbleMenuOverlay

@implementation BubbleMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[BubbleMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - BubbleMenuView
@implementation BubbleMenuView {
    BubbleMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    BubbleMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([BubbleMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [BubbleMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = BubbleMenuViewArrowDirectionUp;
        ...
        _arrowDirection = BubbleMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = BubbleMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = BubbleMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = BubbleMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(BubbleMenuSelectedItem)selectedItem {
    ...
    BubbleMenuOverlay *overlay = [[BubbleMenuOverlay alloc] initWithFrame:view.bounds];
...
- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[BubbleMenuOverlay class]])
    ...
    [BubbleMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    BubbleMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView {
    ...
    UIFont *titleFont = [BubbleMenu titleFont];

    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;

    for (BubbleMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [BubbleMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [BubbleMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == BubbleMenuViewArrowDirectionUp) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };

    } else if (_arrowDirection == BubbleMenuViewArrowDirectionDown) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };

    } else if (_arrowDirection == BubbleMenuViewArrowDirectionLeft) {

        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };

    } else if (_arrowDirection == BubbleMenuViewArrowDirectionRight) {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [BubbleMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([BubbleMenu backgrounColorEffect] == BubbleMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == BubbleMenuViewArrowDirectionLeft ||
        _arrowDirection == BubbleMenuViewArrowDirectionRight) {
...
/// MenuView
static BubbleMenu                      *gMenu;
/// 背景色
static UIColor                      *gTintColor;
/// 字体
static UIFont                       *gTitleFont;
/// 背景色效果
static BubbleMenuBackgrounColorEffect   gBackgroundColorEffect = BubbleMenuBackgrounColorEffectSolid;
/// 是否显示阴影
static BOOL                          gHasShadow = NO;

@implementation BubbleMenu {
    BubbleMenuView *_menuView;
    BOOL         _observing;
}
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = BubbleMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(BubbleMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[BubbleMenuView alloc] init];
